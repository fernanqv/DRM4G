...
class Resource (object):
    ...
    def hosts(self):
        ...
        elif self.features[ 'lrms' ] == 'fedcloud' and os.path.exists( os.path.join(DRM4G_DIR, "var", "fedcloud_pickled")+"_"+self.name ): # and self.features[ 'enable' ] == 'true'
            instances = []
            with open( os.path.join(DRM4G_DIR, "var", "fedcloud_pickled")+"_"+self.name, "r" ) as pf :
                while True :
                    try:
                        instances.append( self.name+"::"+pickle.load( pf ).ext_ip )
                    except EOFError :
                        break
            self.host_list = instances
            return ' '.join( self.host_list )
        else :
