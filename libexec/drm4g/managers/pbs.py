...
class Resource (drm4g.managers.Resource):
    ...
    def dynamicNodes(self):
        out, err = self.Communicator.execCommand('LANG=POSIX %s -x' % (PBSNODES))
        if err: 
    ...
    def queues(self, Host):
        out, err = self.Communicator.execCommand('LANG=POSIX %s -q' % (QSTAT))
...
class Job (drm4g.managers.Job):
    ...
    def jobStatus(self):
        out, err = self.Communicator.execCommand('LANG=POSIX %s %s -x' % (QSTAT, self.JobId))
