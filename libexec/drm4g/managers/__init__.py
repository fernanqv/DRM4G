...
class Resource (object):
    ...
    def staticNodes(self, host_id):
        command_proc = subprocess.Popen('LANC=POSIX gwhost -x %s' % (host_id),
                shell = True,
    ...
    def hostProperties(self):
        ...
        try:
            out, err = self.Communicator.execCommand('LANC=POSIX uname -n -r -p -o')
            if not err:
        ...
        try:
            out, err = self.Communicator.execCommand('LANC=POSIX grep -e \"model name\" -e \"cpu MHz\" /proc/cpuinfo')
            if not err: 
            ...
            self.logger.log(DEBUG, "stderr: " + err)
        try:
            out, err = self.Communicator.execCommand('LANC=POSIX grep -e MemTotal -e MemFree /proc/meminfo')
            if not err:
            ...
            self.logger.log(DEBUG, "stderr: " + err)
        try:
            out, err = self.Communicator.execCommand('LANC=POSIX df -B 1K $HOME')
            if not err:
